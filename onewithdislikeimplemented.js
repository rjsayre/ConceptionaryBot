		...
		sheetTerms.getRows({
			...
			if(rows==undefined || rows==null || rows.length==undefined || rows.length==0){
				...
				for(var row in rows){
					...
					text1=text1+text[0]+" is "+rows[row].definition+" "+rows[row].definition+"\n"+rows[row].author+"\n \n";
				}
						...
						for(var row in rowsM){
							for(var index in authors){
								...
								if(rowsM[row].author.trim()==authors[index].trim()){
									...
									counter=counter+1;
									message=message+text[0]+" is "+rows[row].definition+" "+rowsM[row].definition+"\n"+rowsM[row].author+"\n \n";
								}
							}
						}
						if(message==""){
						...
						for(var row in rowsM){
							for(var index in authors){
								if(rowsM[row].author.indexOf(authors[index])!==-1){
									selectedRows.push(rowsM[row]);
									counter=counter+1;
									message=message+text[0]+" is "+rowsM[row].definition+" "+rowsM[row].definition+"\n"+rowsM[row].author+"\n \n";
								}
							}
						}
						var messageInfo={};
					...
					sheetAuthors.getRows({offset:0,
						'query':"author=\""+text+"\""},function(err2,rows2){
							if(rows2==undefined || rows2==null || rows2.length==undefined || rows2.length==0){
								...
								//here i return
								var categoryMessage="";
								var arrCategory=rows2[0].info.split(",");
			...
			sheetTerms.getRows({offset: 0,
				'query': "term = \""+text[0]+"\""},function(err,rows){
					...
					for(var row in rows){
						...
						message=message+ text[0]+" is "+rows[row].definition+" "+rows[row].definition+"\n"+rows[row].author+"\n \n";}
					}
					messageInfo.data=selectedRows;
					messageInfo.id=id;
					senderToMessages[sender]=messageInfo;
						...
						'query':"term = \""+text[1]+"\""},function(err,rowsM){
							...
							for(var row in rowsM){
								if(selectedAuthors.indexOf(rowsM[row].author)>-1){
									selectedRows.push(rowsM[row]);
									message=message+text[1]+" is "+rows[row].definition+" "+rowsM[row].definition+"\n"+rowsM[row].author+"\n \n";
	...
	function sendTextMessage(sender, text,id,callback) {
		...
		if(text!=null && sender!=null){
			...
			if(text.length>620){
				...
				if(id==0 || id==1 || id==3 || id==7 || id==8 || id==9|| id==10 || id==12){
...
function createQuickReplies(index,size){

	var indexing=(index+2).toString()+"/"+size.toString();
	console.log("\n \n \n \n \n \n i am in "+indexing+"\n \n \n \n \n")
